...
void Generate_Potential(double *Val, int X, int Y, int Z, double value, int name){
  ...
  switch(name){
    ...
    case 3:{
      int C=3*X/4; // center of the atom potential
      double r0=10;
      double r, dx, dy, dz;
      double r6;
      double pot;
    ...
    case 5:{
      double r=3.1;
      int sphere1[]={3*X/4, Y/2, Z/2};
      int sphere2[]={4*X/5, Y/3, Z/2};
      int sphere3[]={3*X/4, 3*Y/4, Z/2};
      for (z=0; z<Z; z++){
        for (y=0; y<Y; y++){
          for (x=0; x<X; x++){
...
// delta has to be less then 0.5, because of dispersion
